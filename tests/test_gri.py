...
def test_gri_calculation(scenario):
...
def test_gri_series():
    ...
    series_data = pd.Series([150, 50, 160, 260, 140, 85])  # Include values in all GRI ranges
...
def test_gri_constant_glucose():
    ...
    result = iglu.gri(series_data)
    assert len(result) == 1
    assert result['GRI'].iloc[0] == 0  # Should be 0 for constant glucose in target range
