...
def test_grade_eugly_basic():
    ...
    assert result.loc[result['id'] == 'subject1', 'GRADE_eugly'].values[0] < \
...
def test_grade_eugly_custom_targets():
    ...
    result2 = grade_eugly(data, lower=80, upper=180)
...
def test_grade_eugly_multiple_subjects():
    ...
    assert result.loc[result['id'] == 'subject2', 'GRADE_eugly'].values[0] < \
           result.loc[result['id'] == 'subject3', 'GRADE_eugly'].values[0]
    # Subject 3 has mixed control (middle euglycemia)
    assert result.loc[result['id'] == 'subject3', 'GRADE_eugly'].values[0] < \
