...
def test_CGMS2DayByDay_calculation(scenario):
    ...
    if 'tz' in kwargs:
        pytest.skip(f"It seems R implementation of CGMS2DayByDay has a bug with tz = {kwargs['tz']}, SKIP this test")
        return

    expected_results = scenario['results']
    expected_interp_data = np.array(expected_results['gd2d'], dtype=np.float64)
    expected_interp_data = np.where(expected_interp_data == None, np.nan, expected_interp_data)
...
def _gd2d_to_df(gd2d, actual_dates, dt0):
    ...
    return df
@pytest.mark.skip(reason="It seems R implementation of CGMS2DayByDay has a bug with gaps, SKIP this test")
