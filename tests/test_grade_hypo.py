...
def test_grade_hypo_calculation(scenario):
...
def test_grade_hypo_series():
    ...
    series_data = pd.Series([150, 75, 160, 65, 140, 85])  # Include some hypoglycemic values
...
def test_grade_hypo_constant_glucose():
    ...
    result = iglu.grade_hypo(series_data)
    assert len(result) == 1
    assert result['GRADE_hypo'].iloc[0] == 0  # Should be 0 for constant glucose above lower bound
    
    # Test with constant glucose below lower bound
    series_data = pd.Series([70] * 10)
    result = iglu.grade_hypo(series_data)
    assert len(result) == 1
    assert result['GRADE_hypo'].iloc[0] == 100  # Should be 100 for constant glucose below lower bound
...
def test_grade_hypo_different_lower():
    ...
    assert result_70['GRADE_hypo'].iloc[0] != result_80['GRADE_hypo'].iloc[0]  # Different lower bounds should give different results
...
def test_grade_hypo_extreme_values():
    ...
    data = pd.DataFrame({
        ...
        'gl': [40, 400, 30, 500]  # Extreme values both below and above normal range
    })
