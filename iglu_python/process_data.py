...
def _convert_to_dataframe(data: Union[pd.DataFrame, pd.Series, list, np.ndarray],
                         glu: Optional[str], timestamp: Optional[str],
                         id: Optional[str]) -> pd.DataFrame:
...
def _find_column(data: pd.DataFrame, column_name: Optional[str], default_name: str,
                original_columns: list, param_name: str) -> str:
    ...
    if column_lower not in data.columns:
        warnings.warn(f"Could not find user-defined {param_name} argument name '{column_name}' in dataset. "
                     f"Available columns: {original_columns}", stacklevel=2)

        if default_name in data.columns:
            raise ValueError(f"Fix user-defined argument name for {param_name}. "
...
def _process_timestamp_column(data: pd.DataFrame, timestamp: Optional[str],
                            original_columns: list, time_parser: Callable) -> pd.DataFrame:
