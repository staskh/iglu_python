...
def mad_glu(data: Union[pd.DataFrame, pd.Series], constant: float = 1.4826) -> pd.DataFrame:
    """
    ...
    for subject in data['id'].unique():
        ...
        mad_val = np.median(np.abs(subject_data['gl'] - np.median(subject_data['gl']))) * constant
