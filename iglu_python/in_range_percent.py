...
def in_range_percent(data: Union[pd.DataFrame, pd.Series], 
    ...
    if isinstance(data, pd.Series):
        ...
        if total_readings == 0:
            return pd.DataFrame(columns=[f'in_range_{min(r)}_{max(r)}' for r in target_ranges])
    ...
    for subject in data['id'].unique():
        ...
        for range_vals in target_ranges:
            min_val, max_val = sorted(range_vals)
            in_range_count = len(subject_data[(subject_data['gl'] >= min_val) & 
                                            (subject_data['gl'] <= max_val)])
            percentages[f'in_range_{min_val}_{max_val}'] = (in_range_count / total_readings) * 100
