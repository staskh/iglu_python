...
def quantile_glu(data: Union[pd.DataFrame, pd.Series], 
                quantiles: List[float] = [0, 25, 50, 75, 100]) -> pd.DataFrame:
    """
    ...
    result = data.groupby('id')['gl'].apply(
        lambda x: pd.Series(np.quantile(x.dropna(), np.array(quantiles)/100), index=quantiles)
    ).unstack().reset_index()
